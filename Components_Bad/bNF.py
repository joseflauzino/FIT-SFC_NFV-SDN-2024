...
while True:
    ...
    if not recv_data[2] in client_control[recv_data[3]]:
        if len(ft_manager.getConnections()) > 3:
            ...
            ft_manager.broadcastMessage(len(recv_data[0]).to_bytes(2, byteorder='big') + recv_data[2].to_bytes(4, byteorder='big') + recv_data[0][:-len(recv_data[0])+14] + nsh_processor.toHeader() + recv_data[0][38:-1] + b'1')
