	...
	def sffServer(self):
		...
		while True:
			...
			if not self.nsh_processor.service_si in client_control[recv_data[3]][recv_data[2]]:
				client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si] = [1, [recv_data[0], 1]]
				index = 1
			else:
				found_flag = False
				for index in range(1, len(client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si])):
					if recv_data[0] == client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][0]:
						...
						client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][0] += 1
						found_flag = True
						break
				...
				for index in range(1, len(client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si])):
					if client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][1] >= majority_parameter:
						...
						if self.entity_addresses[self.nsh_processor.service_spi][target_entity][0] == None:
							self.data_ext_socket.send(client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][0][:-len(client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][0]) + 14] + client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][0][38:] + recv_data[2].to_bytes(4, byteorder='big'))
							...
							for target_address in self.entity_addresses[self.nsh_processor.service_spi][target_entity]:
								self.ft_manager.sendMessage(target_address, len(client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][0]).to_bytes(2, byteorder='big') + recv_data[2].to_bytes(4, byteorder='big') + client_control[recv_data[3]][recv_data[2]][self.nsh_processor.service_si][index][0][:-1] + b'1')
